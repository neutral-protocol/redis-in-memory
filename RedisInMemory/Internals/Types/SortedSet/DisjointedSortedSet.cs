...
namespace Neutral.TestingUtilities.RedisInMemory.Internals.Types.SortedSet
{
    ...
    internal class DisjointedSortedSet
    {
        ...
        internal void JoinRanges(RedisValue[] values)
        {
            ...
            lock (_opLockObj)
            {
                SortedSet<SortedSetRange> existingRangesToMerge =
                    new SortedSet<SortedSetRange>(new SortedSetRangeScoreComparer());
        ...
        internal IEnumerable<SortedSetEntry> RetrieveByScore(double start, double end, Exclude exclude,
            Order order = Order.Ascending, int skip = 0, int take = -1)
        {
            lock (_opLockObj)
            {
                foreach (var range in _ranges)
