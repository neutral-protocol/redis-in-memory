...
namespace Neutral.TestingUtilities.RedisInMemory.Internals.Types
{
    ...
    internal class MemoryStrings
    {
        ...
        internal RedisValueWithExpiry CreateRedisValueWithExpiry(RedisValue value, TimeSpan? expiry)
        {
            ...
            result.GetType().GetField("expiry", BindingFlags.Instance | BindingFlags.NonPublic)
                ?.SetValue(oResult, expiry);
            result.GetType().GetField("value", BindingFlags.Instance | BindingFlags.NonPublic)
                ?.SetValue(oResult, value);
        ...
        internal async Task<RedisValueWithExpiry> GetFromMemoryWithExpiry(string key,
            Func<Task<RedisValueWithExpiry>> retrieval)
        {
        ...
        internal async Task<RedisValue[]> GetFromMemoryMultiAsync(RedisKey[] keys,
            Func<RedisKey[], Task<RedisValue[]>> retrieval)
        {
            if (!keys.Any()) return new RedisValue[0];
